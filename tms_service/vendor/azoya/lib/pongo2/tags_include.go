...
func (node *tagIncludeNode) Execute(ctx *ExecutionContext, writer TemplateWriter) *Error {
	...
	if node.lazy {
		...
		if node.cache {
			if val, err := GetIncludeCache().FromFile(includedFilename); err == nil {
				includedTpl, err := ctx.template.set.FromBytes(val)
				if err == nil {
					if err := includedTpl.ExecuteWriter(includeCtx, writer); err != nil {
						return err.(*Error)
					}
					return nil
				}
...
func tagIncludeParser(doc *Parser, start *Token, arguments *Parser) (INodeTag, *Error) {
	...
	if filenameToken := arguments.MatchType(TokenString); filenameToken != nil {
		...
		// "cache" flag
		includeNode.cache = arguments.Match(TokenIdentifier, "cache") != nil
		...
		if includeNode.cache {
			if val, err := GetIncludeCache().FromFile(includedFilename); err == nil {
				includedTpl, err := doc.template.set.FromBytes(val)
				if err == nil {
