...
type Context struct {
	...
	//ctx，因为很多地方需要用到context.Context，所以这里保存一个对象，后续有好办法再去掉
	ctx context.Context
}
...
func (c *Context) Logger() log.Logger {
	return c.engine.Logger.For(c.Context())
}

// BgLogger 返回Bglogger对象,这个对象不会产生jaeger追踪，只是普通的日志打印
func (c *Context) BgLogger() log.Logger {
	return c.engine.Logger.Bg()
}

// LoggerFactory LoggerFactory
func (c *Context) LoggerFactory() log.Factory {
	return c.engine.Logger
}

// Metrics 返回metrics对象
func (c *Context) Metrics() *Prometheus {
	return c.engine.Metrics
}

// Client 封装了Client对象，用于发送http请求
func (c *Context) Client() *tracing.HTTPClient {
	return c.engine.Client
}

func (c *Context) Configer() config.Configer {
	return c.engine.Configer
}
...
// WithContext 把context.Context赋值给到gin.Context上
func (c *Context) WithContext(ctx context.Context) *Context {
	c.ctx = ctx
	return c
}

// WithSpan 先把span存入context.Context中，再赋值给gin.Context
func (c *Context) WithSpan(span opentracing.Span) *Context {
	c.ctx = opentracing.ContextWithSpan(c.Context(), span)
	return c
}
