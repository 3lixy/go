...
const (
	...
	DBSLAVE = "slave"
	//DBALIASNAME db别名
	TMSDBALIASNAME = "dbtms"
	//DBALIASNAME db别名
	TRACKDBALIASNAME = "dbtrack"
)
...
func InitDb() {
	conns = &AllDBInstance{
		TmsWriteDb:   connDb(TMSDBALIASNAME, DBMASTER),
		TmsReadDb:    connDb(TMSDBALIASNAME, DBSLAVE),
		TrackWriteDb: connDb(TRACKDBALIASNAME, DBMASTER),
		TrackReadDb:  connDb(TRACKDBALIASNAME, DBSLAVE),
	}
}
...
//ConnDb 操作实例
func connDb(aliasName string, dbType string) *gorm.DB {
	c := GetConfig()
	//配置站点 db+master/slave
	db := aliasName + "_" + dbType
	//驱动名称
